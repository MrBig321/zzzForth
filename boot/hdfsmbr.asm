...
; From 0x1BE(decimal:446):
; Partition-entry:
; 	1 byte(bit7 bootable)
;	3 bytes(CHS)
;	1 byte(Partition type)
;	3 bytes(CHS last absolute sector)
;	4 bytes(LBA of first absolult sector in the partition)
;	4 bytes(Number of sectors in the partition)
pe1	db 0x80,  0x00, 0x00, 0x00,  0x0C,  0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00	; 1st Partition Entry
